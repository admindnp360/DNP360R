...
export default function ProfileScreen() {
  ...
  function openEditSupport() {
    ...
    setEditAddress(supportDetails.address);
  ...
  async function handleSaveSupport() {
    setSaving(true);
    try {
      await updateSupportDetails({ phone: editPhone.trim(), email: editEmail.trim(), address: editAddress.trim(), hours: editHours.trim() });
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.sectionHeaderRow}>
            ...
            {isAdmin && (
              <Pressable onPress={openEditSupport} style={[styles.editChip, { backgroundColor: colors.adminBg }]}>
          ...
          {[
            ...
            <View key={row.label} style={[styles.infoRow, { borderBottomColor: colors.border, borderBottomWidth: i < arr.length - 1 ? 1 : 0 }]}>
              <View style={[styles.infoIcon, { backgroundColor: '#E8F5E9' }]}>
                <Feather name={row.icon as any} size={14} color="#004D40" />
              </View>
              <View style={styles.infoContent}>
                <Text style={[styles.infoLabel, { color: colors.mutedForeground }]}>{row.label}</Text>
                <Text style={[styles.infoValue, { color: colors.text }]}>{row.value}</Text>
      ...
      <Modal visible={showSupportModal} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
          <View style={[styles.modalHeader, { borderBottomColor: colors.border }]}>
            ...
            <Pressable onPress={() => setShowSupportModal(false)}><Feather name="x" size={22} color={colors.text} /></Pressable>
          </View>
          <ScrollView contentContainerStyle={{ padding: 16, gap: 14 }}>
            {[
              { label: 'Phone', value: editPhone, setter: setEditPhone },
              { label: 'Email', value: editEmail, setter: setEditEmail },
              { label: 'Office Address', value: editAddress, setter: setEditAddress },
              { label: 'Office Hours', value: editHours, setter: setEditHours },
            ].map(f => (
              <View key={f.label}>
...
const styles = StyleSheet.create({
  ...
  profileEmpId: { color: 'rgba(255,255,255,0.75)', fontSize: 13, fontFamily: 'Inter_500Medium' },
  section: { marginHorizontal: 16, marginTop: 16, borderRadius: 14, borderWidth: 1, overflow: 'hidden' },
  sectionHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingTop: 12, paddingBottom: 8 },
  sectionTitle: { fontSize: 11, fontFamily: 'Inter_600SemiBold', letterSpacing: 0.8 },
  editChip: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 99 },
